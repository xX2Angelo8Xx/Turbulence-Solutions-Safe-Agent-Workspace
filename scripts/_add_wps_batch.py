...
def next_id(prefix: str, rows: list[dict]) -> int:
...
# Calculate next IDs
doc_next = next_id("DOC", wp_rows)
ins_next = next_id("INS", wp_rows)
gui_next = next_id("GUI", wp_rows)
fix_next = next_id("FIX", wp_rows)

print(f"Next IDs: DOC-{doc_next:03d}, INS-{ins_next:03d}, GUI-{gui_next:03d}, FIX-{fix_next:03d}")

# Define all 17 WPs in order
new_wps = []

def add_wp(prefix, name, desc, goal, us, comments="", depends_on="", blockers=""):
    nonlocal doc_next, ins_next, gui_next, fix_next
    counters = {"DOC": "doc_next", "INS": "ins_next", "GUI": "gui_next", "FIX": "fix_next"}
    num = eval(counters[prefix])
    wp_id = f"{prefix}-{num:03d}"
    if prefix == "DOC": doc_next += 1
    elif prefix == "INS": ins_next += 1
    elif prefix == "GUI": gui_next += 1
    elif prefix == "FIX": fix_next += 1
    new_wps.append({
        "ID": wp_id,
        "Category": prefix,
        "Name": name,
