...
Compatibility fixture for SAF-009 tests post-SAF-012.

SAF-009 tests use fake workspace paths like '/workspace' and 'c:/workspace'
...
(the standard folder name used by all SAF-009 tests) when the workspace root
...
def _patch_detect_project_folder_for_fake_workspace():
    ...
    def _detect_with_fallback(workspace_root: Path) -> str:
        try:
            ...
            # SAF-009 tests use fake workspace paths that don't exist on disk.
            # Fall back to "project" — the standard folder name used in all SAF-009 tests.
            return "project"
