import pytest
pytestmark = pytest.mark.skip(reason="criticist.agent.md was removed in the Agent Workbench redesign")

"""
Edge-case tests for DOC-024: criticist.agent.md for Agent Workbench.
...
- Explicit "does NOT fix" language in persona/role
...
class TestFrontmatterExactValues:
    ...
    def test_name_is_exactly_criticist(self):
        """Name must be exactly 'Criticist'."""
...
class TestToolsList:
    ...
    def test_forbidden_tool_absent(self, forbidden):
        ...
        assert forbidden not in tools, (
            f"Forbidden tool '{forbidden}' found. Criticist is read-only."
        )
    ...
    def test_tools_exactly_match_required_set(self):
        ...
        assert not extra, f"Extra tools found: {extra}. Criticist must have read/search only."
...
class TestCrossReferences:
    ...
    def test_fixer_reference(self):
        """Must reference @fixer (who actually fixes issues)."""
        content = _read_content()
        assert "@fixer" in content, "Missing agent cross-reference: '@fixer'"
    ...
    def test_planner_reference(self):
        """Must reference @planner."""
        content = _read_content()
        assert "@planner" in content, "Missing agent cross-reference: '@planner'"
...
# ---------------------------------------------------------------------------
# Persona — read-only / identify-only emphasis
# ---------------------------------------------------------------------------

class TestReadOnlyPersona:
    """Verify persona emphasizes identification, not fixing."""

    def test_body_says_does_not_fix(self):
        """Body must contain explicit 'does not fix' or 'do not fix' language."""
        _, body = _parse_frontmatter(_read_content())
        body_lower = body.lower()
        assert "not fix" in body_lower, (
            "Body must explicitly state the criticist does not fix issues"
        )

    def test_body_says_does_not_edit(self):
        """Body must state the criticist does not edit files."""
        _, body = _parse_frontmatter(_read_content())
        body_lower = body.lower()
    ...
    def test_body_emphasizes_identify(self):
        """Body should use 'identify' language."""
        _, body = _parse_frontmatter(_read_content())
        body_lower = body.lower()
        assert any(kw in body_lower for kw in ["identify", "finding", "report", "document"]), (
            "Body must emphasize identifying/reporting issues"
        )

    def test_description_emphasizes_review_not_fix(self):
        """Frontmatter description should reflect review-only nature."""
        fm, _ = _parse_frontmatter(_read_content())
        desc = str(fm["description"]).lower()
        has_review = any(kw in desc for kw in ["review", "identif", "bug", "security", "flaw", "issue"])
        assert has_review, "Description must reflect review/identify persona"

    def test_role_section_mentions_no_fixing(self):
        """Role section must explicitly state no fixing."""
        _, body = _parse_frontmatter(_read_content())
        ...
        # Strip markdown bold markers for text matching
        role_plain = role_content.replace("**", "").lower()
        assert "not fix" in role_plain or "not edit" in role_plain, (
            "Role section must explicitly state no fixing/editing"
        )
...
class TestConsistencyWithOtherAgents:
    ...
    def test_first_body_line_mentions_agent_name(self):
        ...
        assert "Criticist" in first_line, (
            f"First body line should mention 'Criticist': '{first_line}'"
        )

    def test_body_uses_bold_agent_name(self):
        """Agent name should be bold (**Criticist**) in intro."""
        _, body = _parse_frontmatter(_read_content())
        assert "**Criticist**" in body, "Body should use **Criticist** (bold) in intro"
    ...
    def test_tools_match_brainstormer_pattern(self):
        """Criticist tools should match brainstormer (both are read-only agents)."""
        ...
        c_fm, _ = _parse_frontmatter(_read_content())
        assert set(c_fm.get("tools", [])) == set(b_fm.get("tools", [])), (
            f"Criticist tools {c_fm.get('tools')} should match brainstormer tools "
            f"{b_fm.get('tools')} (both are read-only agents)"
        )

    def test_model_matches_brainstormer(self):
        """Criticist model should match brainstormer (project standard)."""
        brainstormer_file = AGENT_DIR / "brainstormer.agent.md"
        if not brainstormer_file.exists():
            pytest.skip("brainstormer.agent.md not found for comparison")
        b_content = brainstormer_file.read_text(encoding="utf-8")
        b_fm, _ = _parse_frontmatter(b_content)
        c_fm, _ = _parse_frontmatter(_read_content())
        assert c_fm["model"] == b_fm["model"], (
            f"Criticist model '{c_fm['model']}' should match brainstormer model '{b_fm['model']}'"
        )
