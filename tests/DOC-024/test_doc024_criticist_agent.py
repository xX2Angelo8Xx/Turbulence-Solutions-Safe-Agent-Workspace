...
"""
Tests for DOC-024: criticist.agent.md for Agent Workbench.
...
def test_file_exists():
    """criticist.agent.md must exist at the specified path."""
...
def test_frontmatter_forbidden_tools_absent():
    """Edit and execute tools must NOT appear in the criticist's tools list."""
    content = AGENT_FILE.read_text(encoding="utf-8")
    fm, _ = _parse_frontmatter(content)
    tools = fm.get("tools", [])
    present_forbidden = [t for t in FORBIDDEN_TOOLS if t in tools]
    assert not present_forbidden, (
        f"Forbidden edit/execute tools found in criticist tools list: {present_forbidden}. "
        "The Criticist is review-only and must not have edit, execute, or web tools."
    )
...
def test_body_mentions_criticist_role():
    """Body must reference the agent's critical review role."""
    content = AGENT_FILE.read_text(encoding="utf-8")
    _, body = _parse_frontmatter(content)
    body_lower = body.lower()
    assert any(kw in body_lower for kw in ["review", "bug", "security", "flaw", "issue", "finding"]), (
        "Body must describe the criticist/code review persona"
    )
...
def test_body_emphasizes_no_fixing():
    """Body must explicitly state the criticist does NOT fix issues."""
    content = AGENT_FILE.read_text(encoding="utf-8")
    _, body = _parse_frontmatter(content)
    body_lower = body.lower()
    assert "not fix" in body_lower or "do not fix" in body_lower or "does not fix" in body_lower, (
        "Body must explicitly state that the criticist does not fix issues"
    )
