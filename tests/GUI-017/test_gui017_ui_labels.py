...
2. The success message shows "SAE-{folder_name}" in the notification.
3. The duplicate folder check calls check_duplicate_folder with "SAE-{name}".
4. The duplicate error label contains the SAE- prefix.
"""
...
# ---------------------------------------------------------------------------
# Tests: success message shows SAE prefix
# ---------------------------------------------------------------------------

class TestSuccessMessage:
    def test_success_message_contains_sae_prefix(self, tmp_path: Path) -> None:
        """showinfo must include 'SAE-{folder_name}' in the message body."""
        app = _make_app(project_name="MatlabDemo", destination=str(tmp_path))
        created = tmp_path / "SAE-MatlabDemo"
        created.mkdir()
        ...
        assert "SAE-MatlabDemo" in msg, (
            f"Expected 'SAE-MatlabDemo' in success message, got: {msg!r}"
        )

    def test_success_message_does_not_show_bare_name_only(self, tmp_path: Path) -> None:
        """The success message must not use only the bare project name without SAE- prefix."""
        app = _make_app(project_name="TestProject", destination=str(tmp_path))
        created = tmp_path / "SAE-TestProject"
        created.mkdir()
        ...
        _, msg = mock_mb.showinfo.call_args.args
        # Message should reference SAE-TestProject, not just "TestProject"
        assert "SAE-TestProject" in msg, (
            f"Success message missing SAE- prefix: {msg!r}"
        )
...
# ---------------------------------------------------------------------------
# Tests: duplicate folder check uses SAE prefix
# ---------------------------------------------------------------------------

class TestDuplicateFolderCheck:
    def test_duplicate_check_called_with_sae_prefix(self, tmp_path: Path) -> None:
        """check_duplicate_folder must be called with 'SAE-{name}', not just '{name}'."""
        app = _make_app(project_name="MatlabDemo", destination=str(tmp_path))
        ...
        mock_dup.assert_called_once_with("SAE-MatlabDemo", str(tmp_path))
    ...
    def test_duplicate_check_not_called_with_bare_name(self, tmp_path: Path) -> None:
        ...
        assert actual_name_arg == "SAE-Widget", (
            f"Expected 'SAE-Widget', got '{actual_name_arg}'"
        )
        assert actual_name_arg != "Widget", (
            "check_duplicate_folder was called with bare name instead of prefixed name."
        )

    def test_duplicate_error_label_shows_sae_prefix(self, tmp_path: Path) -> None:
        """When a duplicate exists, the error label must mention 'SAE-{name}'."""
        app = _make_app(project_name="MatlabDemo", destination=str(tmp_path))
        ...
        assert any("SAE-MatlabDemo" in t for t in error_texts), (
            f"Expected SAE-MatlabDemo in error label. Got: {error_texts!r}"
        )

    def test_duplicate_error_label_does_not_use_bare_name(self, tmp_path: Path) -> None:
        """Duplicate error label must not say just '{name}' — must include SAE prefix."""
        ...
        assert all("SAE-" in t for t in non_empty), (
            f"Duplicate error label is missing SAE- prefix. Got: {non_empty!r}"
        )
...
class TestEndToEndNaming:
    def test_various_project_names_all_prefixed_in_success_message(self, tmp_path: Path) -> None:
        """Several different project names must all appear with SAE- in success message."""
        names = ["Alpha", "BetaV2", "My_Project123"]
        for name in names:
            app = _make_app(project_name=name, destination=str(tmp_path))
            created = tmp_path / f"SAE-{name}"
            ...
            _, msg = mock_mb.showinfo.call_args.args
            assert f"SAE-{name}" in msg, (
                f"Expected 'SAE-{name}' in success message, got: {msg!r}"
            )
