"""Tests for FIX-030: Bump Version to 2.0.1.

Verifies that all 5 version reference locations have been updated from 2.0.0 to 2.0.1.
"""
...
def test_config_py_version() -> None:
    """VERSION constant in src/launcher/config.py must be 2.0.1."""
...
def test_pyproject_toml_version() -> None:
    """version field in pyproject.toml must be 2.0.1."""
...
def test_setup_iss_version() -> None:
    """MyAppVersion define in src/installer/windows/setup.iss must be 2.0.1."""
...
def test_build_dmg_sh_version() -> None:
    """APP_VERSION in src/installer/macos/build_dmg.sh must be 2.0.1."""
...
def test_build_appimage_sh_version() -> None:
    """APP_VERSION in src/installer/linux/build_appimage.sh must be 2.0.1."""
...
def test_no_old_version_in_version_files() -> None:
    """None of the 5 version files should still contain the old version 2.0.0 in version-assignment lines."""
    files = [
    ...
    for file_path in files:
        ...
        assert not version_lines, (
            f"{file_path.name} still contains old version '{OLD_VERSION}' "
            f"in non-comment lines: {version_lines}"
        )
