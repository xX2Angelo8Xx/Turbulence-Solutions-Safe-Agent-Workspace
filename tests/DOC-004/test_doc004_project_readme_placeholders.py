...
Verifies that both templates/coding/Project/README.md and
...
def test_default_readme_contains_project_name_placeholder():
    """templates/coding README must contain {{PROJECT_NAME}} placeholder."""
    content = _read(DEFAULT_README)
    assert "{{PROJECT_NAME}}" in content


def test_default_readme_h1_uses_placeholder():
    """templates/coding README H1 heading must use {{PROJECT_NAME}}."""
    content = _read(DEFAULT_README)
    assert "# {{PROJECT_NAME}}" in content


def test_default_readme_no_hardcoded_project_heading():
    """templates/coding README must not have a bare '# Project' H1 heading."""
    content = _read(DEFAULT_README)
