...
Compatibility fixture for SAF-020 tests.

SAF-020 tests use fake workspace paths like 'c:/workspace' that do not exist
on disk.  After SAF-012, zone_classifier.detect_project_folder() scans the
real filesystem, so it raises OSError on these fake paths, causing classify()
to return 'deny' instead of 'allow' for project-folder paths.

This fixture patches detect_project_folder to fall back to returning 'project'
(the standard folder name used by all SAF-020 tests) when the workspace root
...
def _patch_detect_project_folder_for_fake_workspace():
    ...
    def _detect_with_fallback(workspace_root):
        try:
            ...
            # SAF-020 tests use fake workspace paths that don't exist on disk.
            # Fall back to "project" — the standard folder name used in tests.
            return "project"
