"""
Tests for DOC-021: tester.agent.md for Agent Workbench.
...
# Tester has full toolset: read, edit, search, execute
REQUIRED_TOOLS = [
    "read_file",
    "create_file",
...
def test_file_exists():
    """tester.agent.md must exist at the specified path."""
...
def test_body_mentions_tester_role():
    """Body must reference the agent's testing role."""
    content = AGENT_FILE.read_text(encoding="utf-8")
    _, body = _parse_frontmatter(content)
    body_lower = body.lower()
    assert any(kw in body_lower for kw in ["test", "edge case", "validate", "behavior", "unit", "integration"]), (
        "Body must describe the testing persona"
    )
