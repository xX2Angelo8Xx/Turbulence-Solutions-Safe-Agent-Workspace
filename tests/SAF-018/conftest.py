...
Compatibility fixture for SAF-018 tests.

SAF-018 tests use fake workspace paths like '/workspace' and 'c:/workspace'
...
(the standard folder name used by all SAF-018 tests) when the workspace root
...
def _patch_detect_project_folder_for_fake_workspace():
    ...
    def _detect_with_fallback(workspace_root: Path) -> str:
        try:
            ...
            # SAF-018 tests use fake workspace paths that don't exist on disk.
            # Fall back to "project" — the standard folder name used in all SAF-018 tests.
            return "project"
